...
class UPDRepostStates(StatesGroup):
    content = State()


@upd_repost_router.callback_query(F.data.startswith('dell_repost_'))
async def delete_repost_process(call: CallbackQuery, state: FSMContext):
    await state.clear()
    repost_id = call.data.replace('dell_repost_', '')
    await delete_repost_by_id(repost_id=repost_id)
    await call.message.delete()

# @upd_repost_router.callback_query(F.data.startswith('edit_note_text_'))
...
# @upd_repost_router.message(UPDNoteStates.content)
